...
- **fold_end is ~half of `trade_log`**: 763 / 1,602 rows (47.6 %) on the 2026-05-19 baseline.  Excluding fold_end is not a fringe filter — it removes nearly half the data.  Sample size remains ample (839 strategy-decided rows), and the negative-excess picture becomes visible only after fold_end is removed.
...
- **Deduped** (default ON, current model only): keeps the latest WF run_id per symbol.  Answers "what does the current model do?"  2026-05-19 baseline (strategy-decided, fold_end excluded): **n=49, cumulative excess = +124.69 %, win-rate-vs-benchmark = 51.0 %**.
- **Raw** (toggle OFF, multi-run history): every weekly `--force` retrain stacks a fresh batch on top of the previous.  Answers "how has the strategy behaved across all model versions ever trained?"  2026-05-19 baseline (strategy-decided, fold_end excluded): **n=839, cumulative excess = −1711.41 %, avg excess = −2.04 % per trade, win-rate-vs-benchmark = 32.4 %**.  Cumulative excess here is the unweighted sum of per-trade `excess_pct` × 100 (see `dashboard/pages/10_Trade_History.py:327` — `strategy_df["excess_pct"].sum() * 100.0`).  Not a portfolio-weighted compound return — every trade contributes its raw excess regardless of size or capital, so the magnitude scales linearly with trade count.  Pinned by `tests/test_trade_log.py::test_benchmark_aggregates_raw_baseline_2026_05_19` (`pytest.approx(-1711.41, abs=2.0)`).

The two answers diverge by ~1836 percentage points on the same fold_end-excluded slice — neither is wrong.  Page 10 defaults to the deduped view to match the existing summary cards' surface, but the toggle is intentional so an operator who wants the multi-run perspective can see it.  The −7 % avg-excess **stop bleed** is the only finding that survives both views (n=20 deduped, n=526 raw — same magnitude, same sign).  When tuning anything that touches dedup logic, the backfill, or SPY ingestion, run `tests/test_trade_log.py::test_benchmark_aggregates_*_baseline_2026_05_19` as a canary — those tests will fail loudly on drift and require an eyeball check before re-pinning.
