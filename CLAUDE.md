...
- **Account-page Refresh is ~5s/position because `get_last_price` always probes live-first** (`dashboard/pages/9_Account.py:_fetch_ibkr_data`, `execution/ibkr_connection.py:get_last_price`): the 2026-06-15 fix priced the Open Positions table off IBKR (`get_last_price` per held symbol) so it reconciles with the headline Unrealized P&L — but `get_last_price` is a 3-tier fallback that *first* requests `reqMarketDataType(1)` live data, which this account has no subscription for, so every symbol waits out the live-snapshot timeout (~5s observed) before falling to `reqMarketDataType(3)` delayed. The calls are also issued **sequentially** in a `for symbol in {...positions}` loop. Net: a 10-position account takes ~50s on a Refresh click (behind an explicit button + spinner, so correct but slow). **Fix options (independent, can do either/both):** (a) skip the doomed tier-1 probe — call `reqMarketDataType(3)` once up front and request delayed snapshots directly, so there's no per-symbol live-timeout wait (cheapest; this account has no real-time subscription so tier-1 never succeeds — but keep the live probe available in `get_last_price` for callers who *do* have one, e.g. add a `market_data_type`/`prefer_delayed` arg rather than deleting tier-1); (b) issue the per-symbol price fetches **concurrently** via `asyncio.gather` instead of the sequential loop, so total latency is ~one snapshot wait, not N. Option (b) alone cuts the 10-position case from ~50s to ~5s; (a)+(b) together gets it near-instant. No correctness impact either way — purely latency. (filed 2026-06-15 after the Account-page price-source fix, commit e653049.)
