...
- **Raw** (toggle OFF, multi-run history): every weekly `--force` retrain stacks a fresh batch on top of the previous.  Answers "how has the strategy behaved across all model versions ever trained?"  2026-05-19 baseline (strategy-decided, fold_end excluded): **n=839, cumulative excess = −20.4 %, win-rate-vs-benchmark = 32.4 %**.

The two answers diverge by ~145 percentage points on the same fold_end-excluded slice — neither is wrong.  Page 10 defaults to the deduped view to match the existing summary cards' surface, but the toggle is intentional so an operator who wants the multi-run perspective can see it.  The −7 % avg-excess **stop bleed** is the only finding that survives both views (n=20 deduped, n=526 raw — same magnitude, same sign).  When tuning anything that touches dedup logic, the backfill, or SPY ingestion, run `tests/test_trade_log.py::test_benchmark_aggregates_*_baseline_2026_05_19` as a canary — those tests will fail loudly on drift and require an eyeball check before re-pinning.
