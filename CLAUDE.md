...
- **Phase B exit-reason waterfall mislabels a stop that fills ABOVE its trigger as `manual_close`** *(code complete 2026-06-26 — awaiting live verification)* (`execution/reconciliation.py:_infer_exit_reason`): the sibling of the gap-through and CB-flatten mislabels above — a third way an off-session bracket fill loses its exit reason. Two gaps combine: (1) the in-session `reqExecutions` poll returns `order_type=None` for an order that has already filled and gone (unlike the Flex path, which resolves `order_type`→`order_lookup`), so step 1 misses it; and (2) a paper STP can fill slightly ABOVE its trigger on a gap-day opening auction, so step 4's directional `exit_px <= stop + tol` rejects it. With both the order-type and the price-match paths defeated, the trade falls through to `manual_close`. **Observed 2026-06-25:** CVX (held from a 6/15 bracket, `order_decisions` id=1560 — entry $179.00, STP $169.60, TP $193.09) exited on the 6/25 open via its STP leg (order 355, `SELL 223 @ stop $169.60`), but the fill printed **$170.21 — $0.61 above the trigger** (the open was $170.22, above the stop; the day's low $169.27 that truly breached it came later), and the exit was captured by the in-session poll (no `order_type`). Stored `exit_reason='manual_close'` (`trade_log` id=2887) for a −$1,960.17 / −4.9% loss that was unambiguously the protective stop. pnl is unaffected; the wrong label pollutes Page 10's exit-reason donut + per-reason excess. Full write-up: `docs/case_studies/losers_2026-06.md` §II.7. **Fix:** new step-6 `bracket_residual` branch — for a long-only position with a KNOWN resting bracket (`get_latest_approved_bracket` returns one), once TP (step 4 up-match), trailing (steps 1/3), CB flatten (step 2) and model-close (step 5) are all ruled out, a loss-side exit (`exit_px < recorded entry`) can only be the protective STP → `exit_reason='stop'`, `source='bracket_residual'`. Also relaxed the step-5 model-close (`signal_flip`) check to fire on `order_type` ∈ {MKT, None} so an off-session CLOSED_LONG always wins over the residual. **Status:** implemented; CVX id=2887 hand-corrected to `exit_reason='stop'`. Test coverage: 4 new in `tests/test_reconciliation.py::TestBracketResidualStop` (stop-fill-above-trigger → stop / CLOSED_LONG-near blocks residual → signal_flip / above-entry unmatched → manual_close / no-bracket loss → manual_close). Full suite: **470 passed** (+ 2 pre-existing baseline-canary fires awaiting the 6/25 daily-review re-pin, unrelated). Verification pending: the next off-session stop that fills at/above its trigger should reconcile `reason=stop source=bracket_residual` (grep the daily/Flex log) instead of `manual_close`.
