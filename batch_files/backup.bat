@echo off
:: Daily backup to external drive (db/trading.db, logs/, config/settings.yaml).
::
...
::
:: Schedule via Windows Task Scheduler (e.g. daily at ~17:00 ET on weekdays).

:: This file lives in batch_files/; cd to the PROJECT ROOT (parent) so every
:: relative path below (.venv, db/trading.db, logs, config) resolves correctly.
cd /d "%~dp0.."
...
:: ---- 3. Copy config/settings.yaml (user overrides; secrets are env-var only) ----
if exist "config\settings.yaml" (
    if not exist "%BACKUP_ROOT%\config" mkdir "%BACKUP_ROOT%\config"
    echo [%date% %time%] Step 3: copying config/settings.yaml >> "%LOG%"
    copy /Y "config\settings.yaml" "%BACKUP_ROOT%\config\settings.yaml" >> "%LOG%" 2>&1
)

:: ---- 4. Prune DB snapshots older than RETENTION_DAYS ----
echo [%date% %time%] Step 4: pruning DB snapshots older than %RETENTION_DAYS% days >> "%LOG%"
forfiles /p "%BACKUP_ROOT%\db_snapshots" /m "trading_*.db" /d -%RETENTION_DAYS% /c "cmd /c del @path" >> "%LOG%" 2>nul

:: ---- 5. Prune local backup logs older than RETENTION_DAYS ----
