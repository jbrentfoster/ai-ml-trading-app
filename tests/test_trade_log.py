...
class TestBenchmarkAggregatesBaseline20260519:
    ...
    def test_benchmark_aggregates_deduped_baseline_2026_06_22(self):
        """Default page view (dedup=ON, active_universe=ON), fold_end excluded.

        Pin date: 2026-06-22.  Tolerances are wide enough to absorb
        ...
        Re-pin note (2026-06-22): 2026-06-21 weekly `--force` retrain (68
        symbols, mean avg Sharpe +0.30, BUY:SELL 1:1.42).  Deduped n 134 -> 128
        (held-excess names rotated out of the active-universe filter — 13 marked
        inactive incl. CVX/COST/AXP), cum_excess +292.0 -> +282.76, win_rate
        53.0 -> 49.2.  A modest rotation-driven cool-down, not a regime break;
        the wider-3.0-stop edge from the 6/15 validation retrain persists in the
        current-model view."""
        m = self._strategy_metrics(dedup=True, active_universe=True)
        assert m["n"]                 == 128,                      (
            f"Row count drifted from 2026-06-22 baseline of 128 — got {m['n']}.  "
            "Likely cause: a new weekly --force retrain has landed.  Eyeball "
            "the new numbers (Page 10 default view) and re-pin this test."
        )
        assert m["cum_excess_pct"]    == pytest.approx( +282.76, abs=2.0)
        assert m["win_rate_vs_bench"] == pytest.approx(   49.2,  abs=0.5)

    def test_benchmark_aggregates_raw_baseline_2026_06_22(self):
        """Multi-run view (dedup=OFF, active_universe=OFF), fold_end excluded.

        Pin date: 2026-06-22.  Same fold_end-excluded slice as the deduped
        ...
        Re-pin note (2026-06-22): the 2026-06-21 weekly `--force` retrain stacked
        a fresh batch of fold rows on top of history (raw keeps every run), so
        raw n 1439 -> 1512 (+73); cum_excess moved sharply less-negative
        (-358.48 -> -126.10) and win_rate 39.1 -> 39.8 as the wider-stop fold
        keeps diluting older drag."""
        m = self._strategy_metrics(dedup=False, active_universe=False)
        assert m["n"]                 == 1512,                     (
            f"Row count drifted from 2026-06-22 baseline of 1512 — got {m['n']}.  "
        ...
        assert m["cum_excess_pct"]    == pytest.approx(-126.10, abs=2.0)
