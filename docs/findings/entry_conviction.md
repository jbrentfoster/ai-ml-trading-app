...
**2026-06-12** — Operator documented a concrete lever proposal that builds on H2-FinBERT, recorded as a CLAUDE.md Enhancement (*"LLM-news-as-threshold-dial"*): (1) zero FinBERT out of the ensemble; (2) derive a per-symbol score from `llm_news_analysis` over the past *x* days; (3) use it as a **threshold dial** on the BUY gate (bullish → lower bar, bearish → raise bar) rather than a score component — which sidesteps both the mis-attribution pollution (LLM path is attribution-gatable) and the conviction-magnitude pollution (news never touches `ensemble_score`, so the H1 margin axis stays clean). Quick data check confirmed it is **not yet testable**: of 14 `source='live'` BUYs entered in the LLM-coverage era (≥2026-06-02), 11 exited via `cb_flatten` (the 2026-06-10 mass liquidation — P&L uninformative about entry quality); only NXPI/ON had strategy-driven exits, so effective testable n ≈ 2. Gated on the same ≥50-matched-live-BUY trigger as this finding, plus enough non-`cb_flatten` LLM-era exits to run a score-vs-forward-excess validation. No code; idea parked.
